...
int main()
{
    fastIO();
    int n, i, x, y;
    int up_total=0, down_total=0, odd_even=0;
    cin >> n;
    for(i=0; i<n; i++){
        cin >> x >> y;
        up_total+=x;
        down_total+=y;
        if((x&1 && !(y&1)) || (!(x&1) && y&1)) odd_even++;
    }
    bool f = true;
    if((up_total+down_total)&1) f = false;
    if(!f) cout << -1;
    else{
        if(!(up_total&1) && !(down_total&1)) cout << 0;
        else if(odd_even>=1) cout << 1;
        else cout << -1; 
    }
